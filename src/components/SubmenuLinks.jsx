...
export function SubmenuLinks({text, path, isActive=false}) {
    return (
        <div>
            <Link to={path}
                  className={`text-decoration-none d-flex  tw-text-xs align-items-center py-2 px-4 tw-rounded-0 tw-group  d-block fw-normal  tw-transition-all tw-duration-300 tw-ease-in-out ${isActive ? 'text-black bg-primary-subtle border-5  rounded-2' : 'text-info'}`}>
